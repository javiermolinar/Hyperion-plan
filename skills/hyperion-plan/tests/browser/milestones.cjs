...
(async()=>{const browser=await launch();try {
  ...
  assert.equal(await s.ui.locator('.pc-next li').count(),2);
  await s.ui.locator('.pc-select-batch').click();
  assert.deepEqual((await s.frame.evaluate(()=>window.__saved)).modelContent.selected_step_ids,['s7','s8']);
  assert.equal(await s.frame.evaluate(()=>window.__calls.length),0,'Suggestion selection never submits');
  ...
  await r.ui.locator('.pc-select-batch').click();await r.ui.locator('.pc-implement').click();
  ...
  console.log('PASS: 27-step milestones, ready batch selection, collapsed navigation and restoration, plan/code review isolation, evidence details, responsive layouts.');
