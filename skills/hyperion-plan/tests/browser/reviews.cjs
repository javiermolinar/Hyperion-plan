...
try{
  ...
  const invalidSaved={modelContent:{kind:'plan-companion',ui_version:3,plan_id:historyBase.plan_id,base_revision:historyBase.revision,operations:[],selected_step_ids:[]},privateContent:{}};
  ...
  await review.getByRole('textbox',{name:'Ask Codex about: Review selection and progress',exact:true}).fill('Preserve the reviewer context in the PR description.');
  ...
  assert.doesNotMatch(fs.readFileSync(dir+'/review-ux-roundtrip-pr-notes.md','utf8'),/Preserve the reviewer context in the PR description/);
  ...
  assert.equal(await restored.ui.getByRole('textbox',{name:'Ask Codex about: Review selection and progress',exact:true}).inputValue(),'Preserve the reviewer context in the PR description.');
  ...
  for(const [width,theme] of [[736,'light'],[320,'dark']]){
    ...
    const expandedHeight=await row.evaluate(e=>e.getBoundingClientRect().height);assert.ok(expandedHeight<(width===736?480:720),`Review too tall: ${expandedHeight}`);
  ...
  assert.deepEqual(errors,[]);console.log('PASS: protected history drafts; independent timing/scope; inherited context; compact settings; one add control; persisted scope/exports; restored drafts; preview isolation. Mock host only.');
