...
**While implementing: save progress at every step boundary.** Checkpoint the selected step as `in_progress` before starting its implementation or review. As soon as its acceptance criteria and relevant checks are satisfied, checkpoint it as `completed` with concise observed evidence. In sequential mode, save that outcome before starting the next step. In parallel mode, checkpoint each start before dispatch and each completion after integration and validation, before starting dependent work. Do not defer these writes to the end of the batch or response, and do not mark every selected step in progress on submission. If verification is still pending or fails, keep the step incomplete and checkpoint the observed result or blocker before moving to other ready work. Record new or cleared blockers when they become known. Handover steps use their transfer lifecycle instead of ordinary checkpoints.

After each saved start, completion, or blocker change, send a concise commentary update naming the step and its actual state; completion updates should include the result and what comes next. Commentary and native task-plan updates do not replace the canonical checkpoint. The inline card is a snapshot, so an existing card does not change when Markdown is saved; show the refreshed card at the end according to the display rules below. During a long step, checkpoint meaningful partial results as they become known without marking it complete; routine tool calls need no checkpoint.
...
| Step start, intermediate completion, or blocker change during implementation | Checkpoint immediately at the step boundary and report the saved state in commentary; the existing card remains a snapshot. |
| Routine tool call | No checkpoint or card needed unless it reveals a meaningful result or blocker. |
...
**First-draft checklist — complete before the first render:**

- Assign an explicit `reasoning_effort` to every implementation and review step using the guidance below. Base the choice on uncertainty, interactions, and the consequences of an error, not file count; preserve explicit user preferences.
- Decide what can run in parallel by inspecting dependencies, shared files, interfaces, and resources. Record actual ordering constraints with `depends_on` and assign the same positive `parallel_group` number (1–30) to steps you assess can safely run together. Use a different number for each group; omit it for sequential work, reviews, and handovers. Groups must not contain dependent steps or span review/handover barriers. Show effort and numbered group badges before execution; do not ask the user to enable subagents.
- Assess whether fresh context would help at a coherent phase boundary, considering accumulated investigation, a change of subsystem, and the cost of losing working context. For a substantial plan, add a `kind: "handover"` checkpoint where useful, or briefly explain in the plan's introductory prose why none is useful. Small, coherent plans need no mechanical checkpoint or written justification.
- Re-read the saved draft before rendering: verify that each implementation and review step has an explicit effort (or a preserved user-selected `inherit`), and that a substantial plan contains either a justified handover checkpoint or the no-handover explanation. Fix omissions before showing the first card.
...
- [ ] Preserve the paused session
  <!-- plan-step: {"reasoning_effort":"high"} -->
...
## Ask Codex about a step

The expanded row uses one **Ask Codex** field instead of the actions menu and separate note editor. Users can ask questions, request splitting or reordering, add a review or handover checkpoint, remove a planned step, or report completion in their own words. The card submits `intent: "ask"`, exactly one stable ID in `target_step_ids`, and `question` (1–1000 characters), with empty `operations` and no implementation selection. Question drafts are separate from unsent plan edits and survive host restoration and failed sends. Enter sends; Command+Enter inserts a line.

Apply the exact request first to check revision and ownership and record its retry receipt. Acceptance changes no step, approval, freshness state, or visible plan revision, so a pure answer does not make the current card stale. Read the step and its dependencies, then answer a pure question without editing the plan. For a clear plan-change request, use targeted CLI commands or `revise`, preserving unrelated decisions, completed history, and valid dependencies. Use the current revision returned by each write. Asking does not authorize implementation, launch a code-review task, or execute a newly added handover. Keep direct instructions from the user authoritative.

For removal, remove only planned work and resolve dependent references as part of the requested change; never interpret it as reverting code. A user-reported completion must use user completion provenance, not invented verification. If a retry is already applied, inspect existing progress and the prior response before continuing: do not repeat an already completed mutation. Refresh the card after plan changes; answer without another card if only explaining. The receipt records submission, not proof the requested change is complete. Older card note and action requests remain supported; never silently discard an old unsent note.
...
Use [Context handovers](references/handovers.md) for `intent: "handover"`, “Continue in fresh task”, recording a mid-step transfer, or resuming as the destination. For large plans, propose explicit `kind: "handover"` steps between coherent phases, preferably after verification or review and before another subsystem. Give each checkpoint a concise reason in its description. Avoid a checkpoint after every small task. It is a movable/removable pending plan item, not implementation work: all preceding steps must finish, later steps wait, and it transfers automatically when an approved run reaches it. Selecting work authorizes the intervening checkpoints and the checkpoint immediately after that batch; the helper records them in `execution.selected_step_ids`. No separate checkpoint click or confirmation is needed. A saved plan alone is not an execution request. Never bypass an incomplete checkpoint. `next` reports ready checkpoints separately in `ready_handover_steps`. Existing `handover_after` markers remain advisory for compatibility; actual `handovers` events preserve where execution moved, including during an in-progress step. Keep one canonical plan and the same checkout. An approved run containing the checkpoint explicitly authorizes its fresh task; an ad-hoc mid-step handover still needs user instruction; do not fork accumulated conversation history. This feature does not monitor context usage or install compaction hooks.

Before mutating a plan, check `execution_owner` in `status`/`show`. Supply `--task-id` with your actual task ID when ownership is set; never use another task's ID to bypass ownership. If another task owns it, direct the user there. Prepare a brief, have the destination check readiness read-only, record the transfer, then ask it to continue existing approved scope. The source stops execution after transfer. The helper completes a handover checkpoint only on ownership transfer; do not mark it done with a status edit or checkpoint command. A cancelled transfer returns it to pending and revokes that checkpoint’s approval; do not automatically retry until a new run authorizes it. A fork is for an explicitly requested alternative branch of work, not the default fresh-context handover. Before starting each next step and after completing a batch, read `next`. If `ready_handover_steps` contains an approved checkpoint, create a request with a fresh request ID, the current revision, `intent: "handover"`, empty operations, and that checkpoint ID in `target_step_ids`; apply it immediately and follow references/handovers.md without asking again. While a handover is unresolved, do not implement. Paused/cancelled scope stays paused/cancelled. Preserve task IDs, brief/evidence paths, current source/destination, and handover request ID through compaction or failure.

## Per-step reasoning effort

Assign an explicit `reasoning_effort` to each new implementation and review step: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, or `ultra`. Storage also accepts `inherit` and defaults to it when omitted for compatibility; omission is not a completed first-draft decision. This is separate from complexity. Choose from levels supported by the selected model, based on uncertainty, coupling, subtle invariants, and failure risk—not file count. Use low for well-understood mechanical edits, medium for ordinary implementation or bounded reviews with clear checks, high for uncertain behavior or reviews of consequential invariants, and higher levels only when the difficulty warrants them. If model support is unknown, record the intended explicit level as a preference and verify support before execution. Do not ask the user to choose effort as part of routine planning. Preserve user-selected preferences, including explicit `inherit`; change them only when the user requests it. Leave legacy plans unchanged unless revising those steps. The card saves a preference; it does not control the current model invocation.

Before executing a step, read its preference from `show`, `next`, or `status`. For an explicit level, check the chosen model/client's supported levels and apply it at the next supported execution boundary: App Server `turn/start.effort`, CLI `-c 'model_reasoning_effort="high"'`, or the desktop task tool's `thinking` parameter when that task creation/follow-up is already authorized. Use the actual requested value. Do not launch an extra task merely to change effort. `inherit` means use the original task setting, not an explicit override left by the preceding step; restore that setting when using a persistent per-turn override. If the current interface cannot change effort or the model does not support the value, disclose that limitation and retain the current setting rather than claiming the preference was applied. A prompt asking for more thought does not change model effort.

Changing only this preference preserves execution scope and freshness. Steps show a compact brain-icon badge beside the title; its dropdown allows optional adjustment on pending steps. Opening a step never changes its effort. The card permits edits on pending steps; recorded preferences are not evidence of the actual effort used. Keep preferences in step context when decomposing or handing off work, revisiting them for the resulting scope.
...
1. Apply the request through the helper first. It validates selections against the latest resulting plan after edits and accepts selection-only requests with no edits. A stale selection-only card with `selection_snapshot` can be reconciled automatically when selected scopes are unchanged; completed work is omitted. Changed scope or stale edits require inspection and reconciliation before retrying. Do not ask for confirmation merely because the revision advanced; ask only for a missing meaningful decision. Actual blockers and missing prerequisites remain enforced. It durably records the receipt and `execution` scope without changing progress statuses. If validation fails, reconcile the plan before any implementation; do not run stale selections.
...
3. Read the selected steps and their notes, identify dependencies, and implement that scope using the per-step checkpoint loop above. If the applied execution mode is `auto` or `parallel`, follow [Parallel execution](references/parallel-execution.md) before dispatching independent implementation steps to subagents; otherwise execute sequentially. Save each start before working or dispatching. Save each verified outcome before moving to the next sequential step or starting dependent work; independent workers in an approved parallel wave may overlap. Follow list order among ready selected steps in sequential mode. In either mode, reviews and handovers are barriers at the user's chosen point. For `kind: "review"`, follow [Independent review checks](references/review-checks.md); selecting review authorizes the review, not automatic fixes. Keep unselected steps for later. If a selected step requires substantial work from an unselected step, explain the dependency and resolve the scope with the user; do not silently include it. Selection does not waive other applicable action boundaries.
...
5. Verify the per-step checkpoints reflect observed results, retain unselected steps and unrelated decisions, and render a fresh card according to the lifecycle above. Use targeted step/note commands for focused edits and `revise` for coordinated structural changes; both retain authorization only for unchanged scope. Removed or changed steps leave the approved scope, and added steps do not inherit authorization. Ordinary progress evidence does not change scope. Request acceptance alone never means work is in progress or complete.

**Execution is model-managed:** new cards send `execution_mode: "auto"` on implementation requests, with no subagent toggle. During planning, choose explicit effort and assess which work can safely overlap. During the approved run, decide whether bounded subagents are useful given independence, coordination cost, and available capacity; follow [Parallel execution](references/parallel-execution.md). The card shows effort and numbered parallel-group badges, not a promise of concurrency. Respect explicit user constraints. Legacy requests with missing mode remain sequential, and explicit sequential/parallel modes remain readable. The coordinator owns checkpoints, integration, and validation. Neither planning nor a candidate label starts work; only approved selected scope may execute.
...
This command can review unselected steps and never changes execution authorization or progress. `revise` can save scope, dependencies, and review evidence together: set `review_state: "current"` and a new, nonempty `review_note` describing the inspection on each reconciled step. The helper applies this evidence after dependent invalidation in the same canonical save. Without new evidence, warnings remain advisory. Scope changes to an in-progress step preserve its status and progress note and set `needs_replanning`; the card offers **Resume with updated scope** for a fresh Run. Never discard earlier work or reset it to pending. Clearing one step does not automatically clear warnings on its dependents. If a review reveals materially different implementation work, explain the scope change and present it for selection instead of treating a previous approval as blanket authorization.
