...
Steps may specify `reasoning_effort`: `inherit` (also the default when omitted), `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, or `ultra`. This is separate from complexity. Codex chooses the initial effort for each new step from levels supported by the selected model: use low for mechanical edits, medium for ordinary implementation, high for complex reasoning or review, and higher levels only when the difficulty warrants them. Use inherit when model support is unknown. Do not ask the user to choose effort as part of routine planning. Preserve user-selected overrides; change them only when the user requests it. Leave legacy plans unchanged unless revising those steps. The card saves a preference; it does not control the current model invocation.
