...
function checkReady(step, available, selected = []) {
  requireValue(
...
async function main() {
  ...
  for (const key of ["init", "revise", "plan-review"].includes(command) ? ["input"] : command === "apply" ? ["request"] : ["render", "migrate"].includes(command) ? ["output"] : [])
